...
export default function Matches() {
    ...
    if (!matchesState.matches.length) {
    ...
    return (
        ...
        >
            ...
            {matchesState.matches.map((match,ind)=>
            <div key={match.homeTeam.replace(" ","")+ind}
            title='Click to see details' 
            className='flex justify-c enter items-center border-b-2 py-3
            ...
            > 
                ...
                lg:items-center   lg:py-3 px-4 flex-col lg:flex-row lg:gap-0 gap-2'>
                    ...
                    lg:w-1/2 '>
                        <div className=''>{match.homeTeam}</div>
                        <div>{match.homeTeamScore?match.homeTeamScore:"0"}</div>
                    </div>
                    <span className='hidden lg:block'>-</span>
                    <div className='flex lg:justify-start justify-end    lg:flex-row 
                    flex-row-reverse lg:items-center gap-2  lg:w-1/2  '>
                        <div className="">{match.awayTeamScore?match.awayTeamScore:"0"}</div>
                        <div className=''>{match.awayTeam}</div>
                    </div>
                </div>
            </div>)}
        </div>
